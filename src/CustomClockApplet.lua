	...
	if fonturl and string.find(fonturl,"^http") then
		...
		elseif lfs.attributes(luadir.."fonts/"..fontfile) ~= nil then
			...
			if not string.find(fonturl,"%.ttf$") and not string.find(fonturl,"%.TTF$")then
				local sink = ltn12.sink.chain(zip.filter(),self:_downloadFontZipFile(appletdir.."CustomClock/fonts/",fonturl))
				req = RequestHttp(sink, 'GET', fonturl, {stream = true, headers = { Connection = "close" }})
			else
				req = RequestHttp(self:_downloadFontFile(appletdir.."CustomClock/fonts/",fontfile,fonturl), 'GET', fonturl, {stream = true, headers = { Connection = "close" }})
			end
...
function _getLocalizedDateInfo(self,time,text)
	...
	if text and string.find(text,"%%d1") then
		...
		text = string.gsub(text,"%%d1",tostring(tonumber(day)))
	end
